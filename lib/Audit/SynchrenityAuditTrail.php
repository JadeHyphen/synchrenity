...
class SynchrenityAuditTrail {
    ...
    // Feature setters
    public function setAlertCallback(callable $callback) { $this->alertCallback = $callback; }
    public function setRoleAccess(array $roles) { $this->roleAccess = $roles; }
    public function setCustomSchema(array $schema) { $this->customSchema = $schema; }
    public function setTenantId($tenantId) { $this->tenantId = $tenantId; }
    public function setEncryptionKey($key) { $this->encryptionKey = $key; }
    public function setGeoProvider(callable $provider) { $this->geoProvider = $provider; }
    public function setApiToken($token) { $this->apiToken = $token; }
    public function enableImmutableStorage($flag = true) { $this->immutable = $flag; }
    public function setRetentionDays($days) { $this->retentionDays = $days; }
    ...
    public function filterLogs($criteria = []) {
        ...
        return array_filter($logs, function($log) use ($criteria) {
            foreach ($criteria as $key => $value) {
                if (!isset($log->$key) || $log->$key != $value) return false;
            }
            return true;
        });
    }
    ...
    public function apiGetLogs($token, $criteria = []) {
        if ($token !== $this->apiToken) return null;
