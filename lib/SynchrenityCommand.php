...
abstract class SynchrenityCommand
{
    ...
    public function setGroup($group) { $this->group = $group; return $this; }
    public function addAlias($alias) { $this->aliases[] = $alias; return $this; }
    ...
    protected function output($data, $format = 'table', $headers = null) {
        if ($this->kernel && method_exists($this->kernel, 'output')) {
            ...
            if ($format === 'json') echo json_encode($data, JSON_PRETTY_PRINT) . "\n";
            elseif ($format === 'table' && $headers) $this->table($headers, $data);
            else print_r($data);
        }
    }
    ...
    protected function log($level, $msg, $context = []) {
        if ($this->kernel && method_exists($this->kernel, 'log')) $this->kernel->log($level, $msg, $context);
        else error_log("[$level] $msg " . json_encode($context));
    }

    // --- ADVANCED: Exit codes ---
    protected function exit($code = 0) { exit($code); }
