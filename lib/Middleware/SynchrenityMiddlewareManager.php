...
class SynchrenityMiddlewareManager {
    ...
    public function setContext(array $context) {
        ...
        foreach ($this->middleware as $mw) {
            if (method_exists($mw, 'setContext')) $mw->setContext($context);
        }
        return $this;
    }
    ...
    protected function triggerHook($event, ...$args) {
        foreach ($this->hooks[$event] ?? [] as $cb) call_user_func_array($cb, $args);
    }
    ...
    public function handle($request) {
        ...
        $next = function($req) use (&$middleware, &$index, &$next, $context, $errorHandlers) {
            if (!isset($middleware[$index])) return $req;
            $mw = $middleware[$index++];
            try {
                if (method_exists($mw, 'before')) $mw->before($req);
                $resp = $mw->handle($req, $next);
                if (method_exists($mw, 'after')) $mw->after($req, $resp);
                return $resp;
            } catch (\Throwable $e) {
                if (method_exists($mw, 'onError')) return $mw->onError($req, $e);
                foreach ($errorHandlers as $handler) return $handler($req, $e);
                throw $e;
            }
        };
