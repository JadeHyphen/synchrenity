...
class SynchrenityAuditMiddleware implements SynchrenityMiddlewareInterface
{
    ...
    {
        $this->log('after', ['request' => $request,'response' => $response]);
        $this->triggerHook('after', $request, $response);
    }
    public function onError($request, $exception)
    {
        $this->log('error', ['request' => $request,'exception' => $exception]);
        $this->triggerHook('error', $request, $exception);
    ...
    public function setComplianceMeta(array $meta)
    {
        $this->complianceMeta = $meta;
    }
    public function getComplianceMeta()
    {
        return $this->complianceMeta;
    }
