...
class SynchrenityWeave
{
    ...
    // Hot reload toggle
    public function setDevMode($dev) { $this->devMode = (bool)$dev; }
    ...
    public function clearCache() { $this->templateCache = []; $this->fragmentCache = []; }
    public function cacheFragment($key, $value) { $this->fragmentCache[$key] = $value; }
    public function getFragment($key) { return $this->fragmentCache[$key] ?? null; }

    // Dependency injection
    public function inject($name, $value) { $this->injected[$name] = $value; }

    // Customizable syntax
    public function setDelimiters($tagOpen, $tagClose, $echoOpen, $echoClose, $commentOpen, $commentClose) {
        $this->tagOpen = $tagOpen; $this->tagClose = $tagClose;
        $this->echoOpen = $echoOpen; $this->echoClose = $echoClose;
        $this->commentOpen = $commentOpen; $this->commentClose = $commentClose;
    }
    ...
    public function setStrictMode($on=true) { $this->strictMode = $on; }
    ...
    // Preprocessing
    public function addPreprocessor($callable) { $this->preprocessors[] = $callable; }
    ...
    public function useVersion($version) { $this->activeVersion = $version; }

    // Access control
    public function setAccessControl($callable) { $this->accessControl = $callable; }
    public function checkAccess($perm) {
        if ($this->accessControl) return call_user_func($this->accessControl, $perm);
        return true;
    }
    ...
    // API
    public function enableApi($on=true) { $this->apiEnabled = $on; }
    public function renderApi($view, $data=[]) {
        if (!$this->apiEnabled) throw new \Exception('API not enabled');
    ...
    public function enableSandbox($on=true, $allowedFunctions=null) {
        ...
        if ($allowedFunctions) $this->allowedFunctions = $allowedFunctions;
    }
    protected function sandboxEval($code, $data) {
        if (!$this->sandboxEnabled) return $this->evaluate($code, $data);
    ...
    public function getDependencyGraph() { return $this->dependencyGraph; }

    // Partial hydration (for modern JS frameworks)
    protected $hydrationEnabled = false;
    public function enableHydration($on=true) { $this->hydrationEnabled = $on; }

    // Pluggable security policies (CSP, XSS audit, auto-escaping)
    protected $securityPolicies = [];
    public function addSecurityPolicy($name, $callable) { $this->securityPolicies[$name] = $callable; }
    ...
    protected $watchers = [];
    public function addWatcher($callable) { $this->watchers[] = $callable; }
    protected function runWatchers($file) {
        foreach ($this->watchers as $watch) call_user_func($watch, $file);
    }
    ...
    public function addStaticAnalysisHook($callable) { $this->staticAnalysisHooks[] = $callable; }
    public function runStaticAnalysis($template) {
        foreach ($this->staticAnalysisHooks as $hook) call_user_func($hook, $template);
    }

    // Custom error templates for graceful fallback
    protected $errorTemplates = [];
    public function setErrorTemplate($type, $view) { $this->errorTemplates[$type] = $view; }
