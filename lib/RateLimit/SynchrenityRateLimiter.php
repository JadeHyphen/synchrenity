...
class SynchrenityRateLimiter
{
    ...
    public function setQuotaShaper($user, callable $cb)
    {
        $this->quotaShapers[$user] = $cb;
    }
    ...
    public function setCircuitBreaker($action, $breaker)
    {
        $this->circuitBreakers[$action] = $breaker;
    }
    ...
    public function addMetricsStreamer(callable $cb)
    {
        $this->metricsStreamers[] = $cb;
    }
    ...
    {
        if ($user && $action) {
            return $this->accessLog[$user][$action] ?? [];
        }
    ...
    public function setPenaltyEscalation($action, $steps)
    {
        $this->penaltyEscalation[$action] = $steps;
    }

    // Quota exhaustion callback
    protected $quotaExhaustionCallback = null;
    public function onQuotaExhaustion(callable $cb)
    {
        $this->quotaExhaustionCallback = $cb;
    }
    ...
    public function isWhitelisted($user)
    {
        return !empty($this->whitelist[$user]);
    }
    public function isBlacklisted($user)
    {
        return !empty($this->blacklist[$user]);
    }
    ...
    public function sync()
    {
        if ($this->syncCallback) {
            call_user_func($this->syncCallback, $this);
        }
    }
    ...
    public function setIpLimit($ip, $limit, $window)
    {
        $this->ipLimits[$ip] = [ 'limit' => $limit, 'window' => $window ];
    }
    public function setUaLimit($ua, $limit, $window)
    {
        $this->uaLimits[$ua] = [ 'limit' => $limit, 'window' => $window ];
    }
    ...
    // Burst/penalty
    public function setBurstLimit($action, $burst)
    {
        $this->burstLimits[$action] = $burst;
    }
    public function setPenalty($action, $seconds)
    {
        $this->penalties[$action] = $seconds;
    }

    // Global/user lockout
    public function lockout($user = null)
    {
        if ($user) {
            $this->lockouts['users'][$user] = true;
        } else {
            $this->lockouts['global'] = true;
        }
    }
    public function unlock($user = null)
    {
        if ($user) {
            unset($this->lockouts['users'][$user]);
        } else {
            $this->lockouts['global'] = false;
        }
    }
    public function isLockedOut($user)
    {
        return $this->lockouts['global'] || !empty($this->lockouts['users'][$user]);
    }

    // Dynamic window
    public function setDynamicWindow($action, callable $cb)
    {
        $this->dynamicWindows[$action] = $cb;
    }

    // Action groups
    public function setActionGroup($group, array $actions)
    {
        $this->actionGroups[$group] = $actions;
    }
    public function getActionGroup($group)
    {
        return $this->actionGroups[$group] ?? [];
    }
    ...
    {
        file_put_contents($file, serialize([$this->data, $this->limits, $this->lockouts]));
    }
    public function import($file)
    {
        if (file_exists($file)) {
            list($this->data, $this->limits, $this->lockouts) = unserialize(file_get_contents($file));
        }
    }
    ...
    {
        return is_array($this->data) && isset($this->limits['default']);
    }
    ...
    public function check($user, $action)
    {
        ...
        if (isset($this->compositeLimits[$compositeKey])) {
            ...
            if (!isset($this->data['composite'][$compositeKey])) {
                $this->data['composite'][$compositeKey] = [];
            }
            ...
            if ($count >= $limit) {
                ...
                if ($this->complianceMode) {
                    $this->log[] = [ 'event' => 'composite_limit', 'key' => $compositeKey, 'count' => $count, 'limit' => $limit, 'time' => $now ];
                }
        ...
        if (!isset($this->accessLog[$user][$action])) {
            $this->accessLog[$user][$action] = [];
        }
        ...
        if ($this->quota && isset($this->quota[$user]) && $this->stats['allowed'] >= $this->quota[$user]) {
            if ($this->quotaExhaustionCallback) {
                call_user_func($this->quotaExhaustionCallback, $user, $action);
            }
        }
        ...
        if ($ip && isset($this->ipLimits[$ip])) {
            ...
            if ($count >= $limit) {
                ...
                if ($this->complianceMode) {
                    $this->log[] = [ 'event' => 'ip_limit', 'ip' => $ip, 'count' => $count, 'limit' => $limit, 'time' => $now ];
                }
        ...
        if ($ua && isset($this->uaLimits[$ua])) {
            ...
            if ($count >= $limit) {
                ...
                if ($this->complianceMode) {
                    $this->log[] = [ 'event' => 'ua_limit', 'ua' => $ua, 'count' => $count, 'limit' => $limit, 'time' => $now ];
                }
        ...
        if ($this->quota && isset($this->quota[$user]) && $this->stats['allowed'] >= $this->quota[$user]) {
            ...
            if ($this->complianceMode) {
                $this->log[] = [ 'event' => 'quota', 'user' => $user, 'quota' => $this->quota[$user], 'time' => time() ];
            }
        ...
        if (isset($this->dynamicWindows[$action])) {
            $window = call_user_func($this->dynamicWindows[$action], $user, $action, $conf);
        }
        ...
        if ($allowed) {
            ...
            if ($this->complianceMode) {
                $this->log[] = [ 'event' => 'allow', 'user' => $user, 'action' => $action, 'count' => $count + 1, 'limit' => $limit, 'time' => $now ];
            }
            ...
            if ($this->complianceMode) {
                $this->log[] = [ 'event' => 'deny', 'user' => $user, 'action' => $action, 'count' => $count, 'limit' => $limit, 'time' => $now ];
            }
