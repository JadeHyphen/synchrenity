...
class SynchrenityKernel
{
    ...
    protected $aliases = [];
    public function alias($alias, $commandName)
    {
    ...
    protected $schedules = [];
    public function schedule($commandName, $cronExpr)
    {
        $this->schedules[] = ['command' => $commandName,'cron' => $cronExpr];
    }
    ...
    {
        return getenv('CI') || getenv('CONTINUOUS_INTEGRATION');
    }

    // --- ADVANCED: Security (restrict commands) ---
    protected $allowedUsers = [];
    public function allowUser($user)
    {
        $this->allowedUsers[] = $user;
    }
    public function isUserAllowed($user)
    {
        return empty($this->allowedUsers) || in_array($user, $this->allowedUsers);
    }
    /**
