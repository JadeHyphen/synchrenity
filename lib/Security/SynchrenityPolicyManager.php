...
class SynchrenityPolicyManager
{
    ...
    // Policy groups
    public function group($group, array $models) { $this->policyGroups[$group] = $models; }
    public function getGroup($group) { return $this->policyGroups[$group] ?? []; }

    // Dynamic policy loading
    public function loadPolicy($model, $file) { require_once $file; }

    // Per-ability deny/allow hooks
    protected $abilityHooks = [ 'allow'=>[], 'deny'=>[] ];
    public function onAllow($ability, $cb) { $this->abilityHooks['allow'][$ability][] = $cb; }
    public function onDenyAbility($ability, $cb) { $this->abilityHooks['deny'][$ability][] = $cb; }

    // Policy audit trail
    public function auditTrail() { return $this->auditTrail; }

    // Policy snapshot/restore
    public function snapshot($name = 'default') { $this->snapshots[$name] = [ 'policies'=>$this->policies, 'cache'=>$this->cache ]; }
    public function restore($name = 'default') { if (isset($this->snapshots[$name])) { $this->policies = $this->snapshots[$name]['policies']; $this->cache = $this->snapshots[$name]['cache']; } }

    // Import/export hooks
    public function onImport($cb) { $this->importExportHooks['import'][] = $cb; }
    public function onExport($cb) { $this->importExportHooks['export'][] = $cb; }
    public function enableCache($on = true) { $this->cacheEnabled = $on; }
    public function clearCache() { $this->cache = []; }

    // Ability aliasing
    public function alias($alias, $ability) { $this->abilityAliases[$alias] = $ability; }

    // Fallback policy
    public function setFallbackPolicy($policyClass) { $this->fallbackPolicy = $policyClass; }

    // Context injection
    public function setContext(array $ctx) { $this->context = $ctx; }
    public function getContext() { return $this->context; }

    // Policy versioning
    public function setPolicyVersion($model, $version) { $this->policyVersions[$model] = $version; }
    public function getPolicyVersion($model) { return $this->policyVersions[$model] ?? null; }
    ...
    public function export($file) {
        foreach ($this->importExportHooks['export'] as $cb) call_user_func($cb, $this);
    ...
    public function import($file) {
        if (file_exists($file)) {
            ...
            foreach ($this->importExportHooks['import'] as $cb) call_user_func($cb, $this);
        }
    }
    ...
    public function healthStats() { return $this->healthStats; }
    ...
    public function allPolicies() { return $this->policies; }
    ...
    {
        if ($this->cacheEnabled && isset($this->cache[$model])) return $this->cache[$model];
        if (!isset($this->policies[$model])) {
            if ($this->fallbackPolicy) {
                $policy = new $this->fallbackPolicy;
                if ($this->cacheEnabled) $this->cache[$model] = $policy;
                return $policy;
            }
            return null;
        }
    ...
    public function authorize($ability, $model, ...$args)
    {
        ...
        // Ability aliasing
        if (isset($this->abilityAliases[$ability])) $ability = $this->abilityAliases[$ability];
        // Context injection
        if (method_exists($policy, 'setContext')) $policy->setContext($this->context);
        // Before event hook
        ...
        if ($result && isset($this->abilityHooks['allow'][$ability])) {
            foreach ($this->abilityHooks['allow'][$ability] as $cb) call_user_func($cb, $user, $ability, $model, $args);
        }
        if (!$result) {
            $this->healthStats['denials']++;
            if (isset($this->abilityHooks['deny'][$ability])) {
                foreach ($this->abilityHooks['deny'][$ability] as $cb) call_user_func($cb, $user, $ability, $model, $args);
            }
