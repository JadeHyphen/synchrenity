...
class SynchrenityCsrf {
    ...
    // Audit trail
    public function auditTrail() { return $this->auditTrail; }

    // Backend abstraction
    protected function &store() {
        if ($this->backend === 'session') return $_SESSION;
        if ($this->backend === 'array') return $this->arrayStore;
    ...
    public function generateOneTimeToken($scope = null, $user = null) {
        ...
        if ($this->bindIp) $meta['ip'] = $_SERVER['REMOTE_ADDR'] ?? null;
        if ($this->bindUa) $meta['ua'] = $_SERVER['HTTP_USER_AGENT'] ?? null;
        if ($scope) $meta['scope'] = $scope;
        if ($user || $this->userKey) $meta['user'] = $user ?: $this->userKey;
    ...
    public function validateOneTimeToken($token, $scope = null) {
        ...
        if ($meta && !$this->isExpired($meta['time'])) {
            if ($this->bindIp && ($meta['ip'] ?? null) !== ($_SERVER['REMOTE_ADDR'] ?? null)) $valid = false;
            elseif ($this->bindUa && ($meta['ua'] ?? null) !== ($_SERVER['HTTP_USER_AGENT'] ?? null)) $valid = false;
            elseif ($scope && ($meta['scope'] ?? null) !== $scope) $valid = false;
            else $valid = true;
        }
        if ($valid) {
            $store[$this->oneTimeTokensKey][$token]['used']++;
            if ($store[$this->oneTimeTokensKey][$token]['used'] >= $this->tokenUsageLimit) unset($store[$this->oneTimeTokensKey][$token]);
            if ($this->replayProtection) $this->usedTokens[$token] = true;
    ...
    // Token usage limit
    public function setTokenUsageLimit($n) { $this->tokenUsageLimit = $n; }

    // Token revocation
    public function revokeToken($token) { $this->revokedTokens[$token] = true; }
    public function isRevoked($token) { return !empty($this->revokedTokens[$token]); }

    // Generate a per-form token
    public function generateFormToken($formId, $user = null) {
        $store = &$this->store();
        if ($this->multiTokenPerForm) {
            if (!isset($store[$this->formTokensKey][$formId])) $store[$this->formTokensKey][$formId] = [];
            $token = bin2hex(random_bytes(32));
            $meta = [ 'time'=>time() ];
            if ($user || $this->userKey) $meta['user'] = $user ?: $this->userKey;
    ...
    public function getFormToken($formId, $user = null) {
        ...
        if ($this->multiTokenPerForm) {
            ...
            if (!isset($store[$this->formTokensKey][$formId])) return [];
            if ($user || $this->userKey) {
                $tokens = [];
                foreach ($store[$this->formTokensKey][$formId] as $token => $meta) {
                    if (($meta['user'] ?? null) === ($user ?: $this->userKey)) $tokens[] = $token;
                }
                return $tokens;
            }
    ...
    public function validate($token, $formId = null, $scope = null, $user = null) {
        ...
        if ($formId) {
            if ($this->multiTokenPerForm) {
                ...
                if ($user || $this->userKey) $valid = $valid && (($meta['user'] ?? null) === ($user ?: $this->userKey));
            } else {
                $valid = isset($store[$this->formTokensKey][$formId]['token']) &&
                    hash_equals($store[$this->formTokensKey][$formId]['token'], $token) &&
                    !$this->isExpired($store[$this->formTokensKey][$formId]['time'] ?? 0);
                if ($user || $this->userKey) $valid = $valid && (($store[$this->formTokensKey][$formId]['user'] ?? null) === ($user ?: $this->userKey));
            }
        } else {
            $valid = hash_equals($this->getToken(), $token) && !$this->isExpired($store[$this->sessionKey.'_time'] ?? 0);
        }
        if ($this->isRevoked($token)) $valid = false;
        if ($this->bindIp && ($_SERVER['REMOTE_ADDR'] ?? null) !== ($store[$this->sessionKey.'_ip'] ?? ($_SERVER['REMOTE_ADDR'] ?? null))) $valid = false;
        if ($this->bindUa && ($_SERVER['HTTP_USER_AGENT'] ?? null) !== ($store[$this->sessionKey.'_ua'] ?? ($_SERVER['HTTP_USER_AGENT'] ?? null))) $valid = false;
        if ($scope && !in_array($scope, $this->scopes)) $valid = false;
        if ($this->replayProtection && isset($this->usedTokens[$token])) $valid = false;
        if ($this->externalVerifier && !call_user_func($this->externalVerifier, $token, $formId, $scope, $user)) $valid = false;
        if ($valid) {
            $this->tokenUsageCounts[$token] = ($this->tokenUsageCounts[$token] ?? 0) + 1;
            if ($this->tokenUsageCounts[$token] >= $this->tokenUsageLimit) $this->revokeToken($token);
            if ($this->replayProtection) $this->usedTokens[$token] = true;
        }
        ...
        if (!$valid) {
            ...
            $this->stats['fails']++;
            if ($this->complianceMode) $this->log[] = [ 'event'=>'fail', 'token'=>$token, 'form'=>$formId, 'scope'=>$scope, 'user'=>$user, 'time'=>time() ];
        } else {
            $this->stats['validates']++;
            if ($this->complianceMode) $this->log[] = [ 'event'=>'pass', 'token'=>$token, 'form'=>$formId, 'scope'=>$scope, 'user'=>$user, 'time'=>time() ];
        }
        return $valid;
    }
    // Configurable hash algorithm
    public function setHashAlgo($algo) { $this->hashAlgo = $algo; }
    public function getHashAlgo() { return $this->hashAlgo; }

    // Per-user tokens
    public function setUserKey($user) { $this->userKey = $user; }
    public function getUserKey() { return $this->userKey; }

    // Token rotation hooks
    public function onRotate($cb) { $this->rotationHooks[] = $cb; }

    // Advanced logging
    public function getLog() { return $this->log; }
    public function clearLog() { $this->log = []; }

    // Compliance mode
    public function enableComplianceMode($on = true) { $this->complianceMode = $on; }
    public function isComplianceMode() { return $this->complianceMode; }

    // External verifier integration
    public function setExternalVerifier($cb) { $this->externalVerifier = $cb; }
    // Batch validation
    ...
    // Token scopes
    public function setScopes(array $scopes) { $this->scopes = $scopes; }
    public function getScopes() { return $this->scopes; }

    // Token binding
    public function bindIp($on = true) { $this->bindIp = $on; }
    public function bindUa($on = true) { $this->bindUa = $on; }

    // Replay protection
    public function enableReplayProtection($on = true) { $this->replayProtection = $on; }
    public function clearUsedTokens() { $this->usedTokens = []; }
    ...
    public function gc() {
        ...
        if (isset($store[$this->oneTimeTokensKey])) {
            foreach ($store[$this->oneTimeTokensKey] as $token => $meta) {
                if ($this->isExpired($meta['time'] ?? 0)) unset($store[$this->oneTimeTokensKey][$token]);
            }
        }
        // Remove expired form tokens
        if (isset($store[$this->formTokensKey])) {
            foreach ($store[$this->formTokensKey] as $formId => $meta) {
                if ($this->isExpired($meta['time'] ?? 0)) unset($store[$this->formTokensKey][$formId]);
            }
        }
    }
    ...
    public function import($file) {
        if (file_exists($file)) {
            ...
            foreach ($data['store'] as $k => $v) $this->store()[$k] = $v;
    ...
    public function replayEvents(array $events) {
        foreach ($events as $e) $this->trigger($e['event'], ...($e['args'] ?? []));
    }
    ...
    // Policy integration
    public function setPolicy($policy) { $this->policy = $policy; }
    public function getPolicy() { return $this->policy; }
    // Stats
    public function stats() { return $this->stats; }

    // Advanced test utilities
    public function fakeArrayStore($data = []) { $this->backend = 'array'; $this->arrayStore = $data; }
    public function clearArrayStore() { $this->arrayStore = []; }
    ...
    // Set token TTL
    public function setTtl($ttl) { $this->tokenTtl = $ttl; }
