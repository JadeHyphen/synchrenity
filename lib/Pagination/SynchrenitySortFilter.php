...
class SynchrenitySortFilter
{
    ...
    public function addFilterStrategy(callable $cb)
    {
        $this->filterStrategies[] = $cb;

        return $this;
    }
    public function addSortStrategy(callable $cb)
    {
        $this->sortStrategies[] = $cb;
    ...
    {
        return [$this->searchTerm, $this->searchFields];
    }
    protected $sort           = [];
    protected $filters        = [];
    ...
    public function apply(array $items)
    {
        ...
        if ($this->searchTerm && $this->searchFields) {
            ...
            $items  = array_filter($items, function ($item) use ($term, $fields) {
                foreach ($fields as $f) {
                    if (isset($item[$f]) && stripos((string)$item[$f], $term) !== false) {
                        return true;
                    }
                }
