...
class SynchrenityOAuth2Provider
{
    ...
    public function addProvider(string $name, array $config): void
    {
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onAddProvider'])) {
                $plugin->onAddProvider($name, $config, $this);
            }
        }
    }
    ...
    public function getAuthUrl($provider, $state = null, $usePkce = false)
    {
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onAuthUrl'])) {
                $plugin->onAuthUrl($provider, $state, $usePkce, $url, $this);
            }
        }
    ...
    public function handleCallback($provider, $code, $state = null)
    {
        ...
        if (!isset($this->providers[$provider])) {
            ...
            foreach ($this->plugins as $plugin) {
                if (is_callable([$plugin, 'onError'])) {
                    $plugin->onError($provider, $code, $state, $this);
                }
            }
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onCallback'])) {
                $plugin->onCallback($provider, $code, $state, $token, $this);
            }
        }
    ...
    public function refreshToken($provider, $refreshToken)
    {
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onRefresh'])) {
                $plugin->onRefresh($provider, $refreshToken, $this);
            }
        }
    ...
    public function getUserInfo($provider, $accessToken)
    {
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onUserInfo'])) {
                $plugin->onUserInfo($provider, $accessToken, $this);
            }
        }
    ...
    public function revokeToken($provider, $accessToken)
    {
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onRevoke'])) {
                $plugin->onRevoke($provider, $accessToken, $this);
            }
        }
    ...
    public function discover($url)
    {
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onDiscover'])) {
                $plugin->onDiscover($url, $this);
            }
        }
