...
class SynchrenityRouter
{
    ...
    // Route tags/metadata
    public function tagRoute($name, $tag)
    {
        $this->routeTags[$name][] = $tag;
    }
    public function setRouteMetadata($name, $meta)
    {
        $this->routeMetadata[$name] = $meta;
    }
    public function getRouteMetadata($name)
    {
        return $this->routeMetadata[$name] ?? [];
    }
    // Route deprecation
    public function deprecateRoute($name, $message = 'Deprecated')
    {
        $this->deprecatedRoutes[$name] = $message;
    }
    public function isRouteDeprecated($name)
    {
        return isset($this->deprecatedRoutes[$name]);
    }
    public function getDeprecationMessage($name)
    {
        return $this->deprecatedRoutes[$name] ?? null;
    }
    // Route health check
    ...
    public function setAccessControl($cb)
    {
        $this->accessControl = $cb;
    }
    // Route statistics
    public function incrementRouteStat($name, $key = 'hits')
    {
        $this->routeStats[$name][$key] = ($this->routeStats[$name][$key] ?? 0) + 1;
    }
    public function getRouteStats($name)
    {
        return $this->routeStats[$name] ?? [];
    }
    // Route aliasing
    public function alias($alias, $name)
    {
        if (isset($this->namedRoutes[$name])) {
            $this->namedRoutes[$alias] = $this->namedRoutes[$name];
        } $this->routeAliases[$alias] = $name;
    }
    public function resolveAlias($alias)
    {
        return $this->routeAliases[$alias] ?? $alias;
    }
    // Route import/export
    public function exportRoutes($file)
    {
        file_put_contents($file, json_encode($this->routes, JSON_PRETTY_PRINT));
    }
    public function importRoutes($file)
    {
        if (file_exists($file)) {
    ...
    public function testRoute($method, $uri, $expected)
    {
    ...
    public function setParamDefault($param, $value)
    {
        $this->paramDefaults[$param] = $value;
    }
    // Route parameter type casting
    public function setParamCast($param, $type)
    {
        $this->paramCasts[$param] = $type;
    }
    // Route versioning
    public function setRouteVersion($name, $version, $route)
    {
        $this->routeVersions[$name][$version] = $route;
    }
    public function useRouteVersion($name, $version)
    {
        if (isset($this->routeVersions[$name][$version])) {
            $this->namedRoutes[$name] = $this->routeVersions[$name][$version];
        }
    }
    // Route event hooks
    ...
    public function allNamedRoutes()
    {
    ...
    public function dispatch($method, $uri)
    {
        ...
        foreach ($this->routes as $route) {
            // Domain check
            if (isset($route['domain']) && isset($_SERVER['HTTP_HOST']) && $route['domain'] !== $_SERVER['HTTP_HOST']) {
                continue;
            }
        ...
        if ($allowed) {
            ...
            if ($this->methodNotAllowedHandler) {
                return call_user_func($this->methodNotAllowedHandler, $method, $uri, $allowed);
            }
        ...
        if ($this->notFoundHandler) {
            return call_user_func($this->notFoundHandler, $method, $uri);
        }
    ...
    public function setNotFoundHandler($cb)
    {
        $this->notFoundHandler = $cb;
    }
    public function setMethodNotAllowedHandler($cb)
    {
        $this->methodNotAllowedHandler = $cb;
    }
