...
class SynchrenityErrorHandler
{
    ...
    public function handle($error)
    {
        ...
        if ($this->isRateLimited($type, $errorId)) {
            ...
            foreach ($this->plugins as $plugin) {
                if (is_callable([$plugin, 'onRateLimited'])) $plugin->onRateLimited($error, $this);
            }
            return;
        }
        // Suppression
        if ($this->isSuppressed($type, $errorId)) return;
        // Custom handler
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onHandled'])) $plugin->onHandled($error, $this);
        }
    ...
    protected function isRateLimited($type, $errorId) {
        ...
        if (count($this->rateLimits[$type]) > $maxPerType) return true;
        // Per IP
        $this->rateLimitsIp[$ip] = array_filter($this->rateLimitsIp[$ip] ?? [], fn($t) => $t > $now - $window);
        $this->rateLimitsIp[$ip][] = $now;
        if (count($this->rateLimitsIp[$ip]) > $maxPerIp) return true;
        // Per session
        ...
        if (count($this->rateLimitsSession[$session]) > $maxPerSession) return true;
        // Per errorId
        $this->errorIds[$errorId] = array_filter($this->errorIds[$errorId] ?? [], fn($t) => $t > $now - $window);
        $this->errorIds[$errorId][] = $now;
        if (count($this->errorIds[$errorId]) > $maxPerError) return true;
        return false;
    }
    // Error suppression (manual or auto)
    public function suppress($type, $errorId) { $this->suppressed[$type][$errorId] = true; }
    protected function isSuppressed($type, $errorId) { return !empty($this->suppressed[$type][$errorId]); }
    // Error replay (re-handle suppressed/deduped errors)
    public function replay($error) { $this->handle($error); }
