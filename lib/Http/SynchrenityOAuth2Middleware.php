...
class SynchrenityOAuth2Middleware {
    ...
    public function __construct($oauth2Provider, $requiredScopes = [], $options = []) {
        ...
        if (isset($options['jwkSet'])) $this->jwkSet = $options['jwkSet'];
        if (isset($options['jwtDecoder'])) $this->jwtDecoder = $options['jwtDecoder'];
        if (isset($options['providers'])) $this->providers = $options['providers'];
    }
    ...
    public function handle($request, $next) {
        ...
        if (is_callable([$provider, 'introspect'])) {
            ...
            if (is_array($introspect)) $tokenInfo = array_merge($tokenInfo, $introspect);
        }
        // Scope check
        ...
        if ($this->jwtDecoder && is_callable($this->jwtDecoder)) {
            ...
            if (is_array($jwtInfo)) $tokenInfo = array_merge($tokenInfo, $jwtInfo);
        }
        // Hooks/plugins
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'beforeNext'])) $plugin->beforeNext($request, $tokenInfo, $this);
        }
    ...
    protected function logEvent($event, $data) {
        if ($this->complianceMode) $this->log[] = [ 'event'=>$event, 'data'=>$data, 'time'=>time() ];
    }

    protected function hasRequiredScopes($tokenInfo) {
        if (!isset($tokenInfo['scopes'])) return false;
        foreach ($this->requiredScopes as $scope) {
            if (!in_array($scope, $tokenInfo['scopes'])) return false;
        }
        return true;
    }
    ...
    // Introspection
    public function getRequiredScopes() { return $this->requiredScopes; }
    public function getProvider() { return $this->oauth2Provider; }
