...
class SynchrenityApiRateLimitMiddleware {
    ...
    public function handle($request, $next) {
        ...
        try {
            ...
            foreach ($this->plugins as $plugin) {
                if (is_callable([$plugin, 'onError'])) $plugin->onError($request, $e, $this);
            }
        ...
        if (!$allowed) {
            ...
            foreach ($this->plugins as $plugin) {
                if (is_callable([$plugin, 'onBlocked'])) $plugin->onBlocked($request, $this);
            }
        ...
        foreach ($this->plugins as $plugin) {
            if (is_callable([$plugin, 'onAllowed'])) $plugin->onAllowed($request, $this);
        }
    ...
    protected function setRateLimitHeaders($response, $user, $role, $endpoint, $policy) {
        if (!method_exists($response, 'header')) return;
