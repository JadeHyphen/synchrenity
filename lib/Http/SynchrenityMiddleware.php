...
class SynchrenityMiddleware
{
    ...
    {
        if (!$this->enabled) return $next ? $next($request) : $request;
        $this->metrics['calls']++;
        try {
            ...
            foreach ($this->plugins as $plugin) {
                if (is_callable([$plugin, 'before'])) $request = $plugin->before($request, $this);
            }
            ...
            foreach ($this->plugins as $plugin) {
                if (is_callable([$plugin, 'after'])) $response = $plugin->after($response, $this);
            }
    ...
    // Advanced context
    public function setContextArray(array $ctx) { $this->context = $ctx; }
    public function getContextAll() { return $this->context; }

    // Hook registration
    public function before(callable $cb) { $this->beforeHooks[] = $cb; }
    public function after(callable $cb) { $this->afterHooks[] = $cb; }
    public function onError(callable $cb) { $this->errorHooks[] = $cb; }
    ...
    // Introspection
    public function getBeforeHooks() { return $this->beforeHooks; }
    public function getAfterHooks() { return $this->afterHooks; }
    public function getErrorHooks() { return $this->errorHooks; }
